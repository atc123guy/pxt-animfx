...
- `animFX.combineAnimations(a, b, c, ...)` — concatenates multiple animation variables into one sequential frame array. Useful for stringing together "wind up + swing + recover" or similar multi-stage motions and then playing them as a single animation. Click the **+** on the block to add more slots (up to 6 total). Frames are referenced, not cloned — if you also flip one of the source animations, do that *before* combining.
